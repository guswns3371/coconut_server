...
Chat.makeChatRoom = async function(chatRoom,result){
    ...
    if (room_id !== null && typeof room_id !== "undefined"){
        ...
        const findChatRoom = await models.ChatRoom.findOne({where: {id: room_id}});
        if (findChatRoom){
            // 존재하는 방
            _room_id_ = findChatRoom.id;

            const user_chat_room_info_0 = await models.UserChatRoom.findOne({
        ...
        const findChatRoom = await models.ChatRoom.findAll();
        ...
        const room__data = await models.ChatRoom.findOne({where:{people : `${room_people}`}});
       ...
       // console.log(room__data.dataValues.people)
        ...
        if (room__data) {
            ...
            if (room_id === undefined){
               ...
               // UserChatRoom에 기록
                const createUser_ChatRoom = await models.UserChatRoom.findOne({
                    where : {user_id : user_id,chat_room_id : _room_id_}
                });

                if (createUser_ChatRoom === null){
                    // UserChatRoom에 저장이 안된 방
                     const user_char_room_info =   await models.UserChatRoom.create({
           ...
           // console.log({room_people});
            ...
            const createChatRoom = await models.ChatRoom.create({
                people: `${room_people}`
            });
                ...
                for (const roomUserId of room_people){
                    ...
                    for(const room_user_id of room_people){
                        if (room_user_id !== roomUserId || room_people.length === 1){
                            const roomUserInfo = await models.UserInfo.findByPk(room_user_id);
                    ...
                    // UserChatRoom에 기록
                    const createUser_ChatRoom = await models.UserChatRoom.count({
                        where : {
                            user_id : roomUserId,
                    ...
                    if (createUser_ChatRoom === 0){
                        // UserChatRoom에 저장이 안된 방
                      const user_chat_room_info_2 = await models.UserChatRoom.create({
                            user_id : roomUserId,
                            chat_room_id : _room_id_,
                            room_name : roomName
                        });
    ...
    if (history){
        ...
        for (const his of history) {
           ...
           // historyData.read_people = `${historyData.read_people}`;
            ...
            const user = await models.UserInfo.findByPk(historyData.chat_user_id);
...
Chat.getChatRoomList = async function(userId,result){
    ...
    // 유저가 들어간 채팅방 id 정보들
    const chatRoomList = await models.UserChatRoom.findAll({
    ...
    for (const room of room_info_list){
        room_infos = await models.ChatRoom.findByPk(room.chat_room_id);
...
Chat.makeHistory = async function(req,result){
    ...
    const history = await models.ChatHistory.create({
    ...
    if (history){
        const chatRoom = await models.ChatRoom.findOne({where : { id : body.chat_room_id}});
